...
HotlineMiamiBaseScene::HotlineMiamiBaseScene(std::string name)
	: Scene(name)
{
	//No gravity, this is hotline miami
	m_gravity = b2Vec2(0.f, 0.f);
	m_physicsWorld->SetGravity(m_gravity);
}

void HotlineMiamiBaseScene::InitScene(float windowWidth, float windowHeight)
{
	//Effect stuffs
	...
	//Setup new Entity
	{
		...
		ECS::GetComponent<Transform>(entity).SetPosition(vec3(-50.f, 5.f, 100.f));
...
void HotlineMiamiBaseScene::Update()
{
...
void HotlineMiamiBaseScene::RainbowBackground()
{
	//Change m_clearcolor using lerp
...
void HotlineMiamiBaseScene::AdjustScrollOffset()
{
...
void HotlineMiamiBaseScene::Aiming(vec2 mousePosition)
{
...
void HotlineMiamiBaseScene::KeyboardHold()
{
	if (ECS::GetComponent<EntityIdentifier>(EntityIdentifier::MainPlayer()).GetIdentifier()& EntityIdentifier::PhysicsBit())
	{
		auto body = ECS::GetComponent<PhysicsBody>(EntityIdentifier::MainPlayer()).GetBody();
...
void HotlineMiamiBaseScene::MouseMotion(SDL_MouseMotionEvent evnt)
{
	m_mouseGL = Util::ConvertToGL(m_sceneReg, vec2(float(evnt.x), float(evnt.y)));
}

void HotlineMiamiBaseScene::MouseClick(SDL_MouseButtonEvent evnt)
{
}
